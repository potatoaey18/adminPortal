...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM conversations_admin_hte WHERE supervisor_id = ? AND admin_id = ?");
    $stmt->execute([$supervisor_id, $admin_id]);
    $conversation = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$conversation) {
        // Create a new conversation if it doesn't exist
        $stmt = $conn->prepare("INSERT INTO conversations_admin_hte (supervisor_id, admin_id, created_at) VALUES (?, ?, NOW())");
        $stmt->execute([$supervisor_id, $admin_id]);
    ...
    $stmt = $conn->prepare("SELECT user_id FROM users_admin_hte WHERE email = ? AND user_type = 'admin'");
    ...
    $stmt = $conn->prepare("INSERT INTO messages_admin_hte (conversation_id, sender_id, sender_type, content, created_at) VALUES (?, ?, 'admin', ?, NOW())");
    ...
    $stmt = $conn->prepare(
        ...
        LEFT JOIN users_admin_hte u ON m.sender_id = u.user_id
        WHERE m.id = ?"
    );
    ...
    if (defined('PUSHER_APP_KEY') && !empty(PUSHER_APP_KEY)) {
        try {
            ...
            $stmt = $conn->prepare("SELECT first_name, last_name FROM supervsior WHERE id = ?");
            $stmt->execute([$supervisor_id]);
            $hte = $stmt->fetch(PDO::FETCH_ASSOC);
            $hte_name = $hte ? trim($hte['first_name'] . ' ' . $hte['last_name']) : 'HTE';
