...
if (isset($_POST['action']) && isset($_POST['id']) && isset($_POST['csrf_token'])) {
    // Basic CSRF token check (implement a real token system in production)
    if ($_POST['csrf_token'] !== 'your_csrf_token_here') { // Replace with a dynamic token
    ...
    try {
        ...
        if ($user) {
            if ($action === 'verify') {
                ...
                if ($role === 'admin') {
                    $stmt = $conn->prepare("INSERT INTO admin_accounts (uniqueID, first_name, middle_name, last_name, id_number, address, gender, age, phone_number, email, role, password, access_level) 
                                            VALUES (:uniqueID, :first_name, :middle_name, :last_name, :id_number, :address, :gender, :age, :phone_number, :email, :role, :password, :access_level)");
                    $stmt->execute([
                    ...
                    $stmt = $conn->prepare("INSERT INTO coordinators_account (uniqueID, first_name, middle_name, last_name, id_number, address, gender, age, phone_number, email, role, password, access_level) 
                                            VALUES (:uniqueID, :first_name, :middle_name, :last_name, :id_number, :address, :gender, :age, :phone_number, :email, :role, :password, :access_level)");
                    $stmt->execute([
                        ':uniqueID' => $user['uniqueID'],
                        ':first_name' => $user['first_name'],
                    ...
                    $stmt = $conn->prepare("INSERT INTO supervisor (uniqueID, first_name, middle_name, last_name, id_number, address, phone_number, email, role) 
                                            VALUES (:uniqueID, :first_name, :middle_name, :last_name, :id_number, :address, :phone_number, :email, :role)");
                    $stmt->execute([
                        ':uniqueID' => $user['uniqueID'],
                        ':first_name' => $user['first_name'],
                        ':middle_name' => $user['middle_name'],
    ...
    <div class="content-wrap" style="height: 80%; width: 100%; margin: 0 auto;">
        <div style="background-color: white; margin-top: 6rem; margin-left: 16rem; padding: 2rem;">
            ...
            <div class="table-container">
                <?php
                try {
                    $stmt = $conn->prepare("SELECT * FROM pending_users WHERE role IN ('admin', 'coordinator', 'supervisor') ORDER BY role");
                    $stmt->execute();
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            data: {
                                action: 'verify',
                                id: id,
                                csrf_token: 'your_csrf_token_here' // Replace with a dynamic token
                            },
