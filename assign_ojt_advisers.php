...
if (isset($_POST['ajaxDeleteSection'])) {
    ...
    try {
        ...
        $conn->commit();
        error_log("AJAX Delete success: Combined Section=$combinedSection, ID=$id");
...
if (isset($_POST['ajaxSaveAssignment'])) {
    ...
    error_log("AJAX Save Assignment: Section ID=$section_id, Adviser ID=$adviser_id");

    if (empty($section_id) || $section_id <= 0 || empty($adviser_id) || $adviser_id <= 0) {
        error_log("AJAX Error: Invalid section ID or adviser_id.");
        echo json_encode(['status' => 'error', 'message' => 'Valid section ID and adviser ID are required.']);
        exit;
    }
    ...
    try {
        ...
        // Fetch course and section from courses_sections
        $stmt = $conn->prepare("SELECT course, section FROM courses_sections WHERE id = ?");
        $stmt->execute([$section_id]);
        $sectionData = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$sectionData) {
            error_log("AJAX Error: Section not found for ID=$section_id");
            echo json_encode(['status' => 'error', 'message' => 'Section not found.']);
            exit;
        }
        $course = $sectionData['course'];
        $section = $sectionData['section'];
        $combinedSection = "$course $section"; // e.g., "DIT 3-2"
        ...
        $isAssignedToThisSection = in_array($combinedSection, array_filter([
        ...
        if ($coordinator['assigned_section'] === $combinedSection) {
        ...
        } elseif ($coordinator['second_assigned_section'] === $combinedSection) {
        ...
        if (empty($coordinator['assigned_section'])) {
            ...
            $stmt->execute([$combinedSection, $adviser_id]);
            error_log("Set assigned_section: Combined Section=$combinedSection, Adviser ID=$adviser_id");
            ...
            $stmt->execute([$combinedSection, $adviser_id]);
            error_log("Set second_assigned_section: Combined Section=$combinedSection, Adviser ID=$adviser_id");
        }

        $conn->commit();
        error_log("AJAX Success: Adviser ID=$adviser_id assigned to combined section $combinedSection");
        ...
        <div>
            ...
            <div class="add-course-section">
                <form method="POST" action="" id="addCourseSectionForm">
                    ...
                    <input type="text" name="section" placeholder="Year & Section (e.g., 3-2)" required>
                    <button type="submit" name="addCourseSection">Add Course & Section</button>
                </form>
            </div>
            <div class="search-container">
                <input type="text" class="search-box" placeholder="Search by section (e.g., DIT 3-2)..." id="sectionSearch" aria-label="Search sections">
                        ...
                        <table class="company-table" id="courseTable_<?php echo htmlspecialchars(str_replace(' ', '_', $course)); ?>">
                            ...
                            <tbody>
                                <?php foreach ($courseSections as $section): ?>
                                    <?php $combinedSection = htmlspecialchars($section['course'] . ' ' . $section['section']); ?>
                                    <tr data-section-id="<?php echo htmlspecialchars($section['id']); ?>">
                                        <td><?php echo $combinedSection; ?></td>
                                        <td>
                                            <form class="assignment-form">
                                                ...
                                                <select name="adviser_id" class="adviser-select" 
                                                        <?php echo isset($currentAssignments[$combinedSection]) ? 'disabled' : ''; ?>>
                                                    <option value="">Select Adviser</option>
                                                    <?php foreach ($advisers as $adviser): 
                                                        $assignedCount = $adviserAssignments[$adviser['id']]['assigned_count'];
                                                        $isAssignedToThisSection = in_array($combinedSection, $adviserAssignments[$adviser['id']]['assigned_sections']);
                                                        $isDisabled = ($assignedCount >= 2 && !$isAssignedToThisSection) ? 'disabled' : '';
                                                    ?>
                                                        <option value="<?php echo htmlspecialchars($adviser['id']); ?>" 
                                                                <?php echo (isset($currentAssignments[$combinedSection]) && $currentAssignments[$combinedSection] == $adviser['id']) ? 'selected' : ''; ?>
                                                                <?php echo $isDisabled; ?>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="action-btn edit-adviser-btn" 
                                                    ...
                                                    data-adviser-id="<?php echo isset($currentAssignments[$combinedSection]) ? htmlspecialchars($currentAssignments[$combinedSection]) : ''; ?>">Edit Adviser</button>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="editAdviserForm">
                    <div class="modal-body">
                        <input type="hidden" name="section_id" id="editAdviserSectionId">
                        <div class="mb-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit-adviser-btn', function(e) {
                e.preventDefault();
                console.log('Edit adviser button clicked:', $(this).data());
                const sectionId = $(this).data('section-id');
                const currentAdviserId = $(this).data('adviser-id');

                if (!sectionId) {
                    console.error('Missing section ID data');
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Missing section ID data.',
                        confirmButtonColor: '#3085d6'
                    });
                    return;
                }

                $('#editAdviserSectionId').val(sectionId);
            ...
            $(document).on('click', '.delete-btn', function(e) {
                e.preventDefault();
                console.log('Delete button clicked:', $(this).data());
            ...
            function saveAdviserAssignment($select) {
                ...
                const sectionId = $form.find('input[name="section_id"]').val() || $('#editAdviserSectionId').val();
                const adviser_id = $select.val();

                console.log('Saving adviser assignment:', { sectionId, adviser_id });

                if (!sectionId || !adviser_id || adviser_id === "") {
                    console.warn('Missing section ID or adviser_id');
                    Swal.fire({
                        icon: 'warning',
                        title: 'Warning',
            ...
            $(document).on('click', '.edit-btn', function(e) {
                e.preventDefault();
                console.log('Edit course/section button clicked:', $(this).data());
                const id = $(this).data('id');
